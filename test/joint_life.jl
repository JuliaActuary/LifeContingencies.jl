@testset "Joint Life" begin
    ...
    @testset "Joint Last Survivor unknown status" begin
        ...
        @testset "ALMCR §9.4" begin
            ...
            @test cumulative_decrement(m1, 65, 66) == 1 - ℓ₁[2] / ℓ₁[1]
            @test cumulative_decrement(m2, 60, 61) == 1 - ℓ₂[2] / ℓ₂[1]
        ...
        @testset "CIA tables" begin
            ...
            @testset "precalced vectors" begin
                for time in 1:40
                    @test isapprox(cumulative_decrement(jl,time), q_cumulative[time], atol = 1e-6)
                end
